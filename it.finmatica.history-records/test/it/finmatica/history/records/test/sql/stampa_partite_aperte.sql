...
SELECT
(SELECT NVL(C_BPartner.Value,'-1') ||'_'|| NVL(C_BPartner.Name,'-1') FROM C_BPartner WHERE RV_OpenItem.C_BPartner_ID=C_BPartner.C_BPartner_ID) AS AC_BPartner_ID,
...
(SELECT NVL(C_Activity_Trl.Name,'-1') FROM C_Activity INNER JOIN C_Activity_TRL ON (C_Activity.C_Activity_ID=C_Activity_Trl.C_Activity_ID AND C_Activity_Trl.AD_Language='it_IT') WHERE RV_OpenItem.C_Activity_ID=C_Activity.C_Activity_ID) AS GC_Activity_ID,
